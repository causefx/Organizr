...
$element.find('.icp-auto-pend').iconpicker({placement: 'left', hideOnSelect: false, collision: true}
).hide();
  ...
  <div id="main-wrapper" class="main-wrapper">
    ...
    <div id="content"  style="margin:0 10px; overflow:hidden">
      ...
      <div class="row">
        ...
        if ($userDevice !== "phone") {
            echo "<br><br><br>".buildMenu($buildMenu);
        } else {
            echo buildMenuPhone($buildMenu);
        }
        ?>
    ...
    <div class="email-content homepage-box white-bg">
      ...
      $urlPattern = '([hH][tT][tT][pP][sS]?):\/\/([\w\.\-]{1,250}
      )(?::(\d{1,5}
      ))?((?:\/[^?.\s]+))?';
    ...
    array(
      ...
      'tabs' => array(
        array(
          ...
          'fields' => array(
            array(
              ...
              'name' => 'authType',
              'value' => AUTHTYPE,
              'onchange' => 'if (this.value == \'internal\') { $(\'.be-auth, #authBackend_id, #authBackendCreate_id\').parent().hide(); }
              else { $(\'#authBackend_id, #authBackendCreate_id\').trigger(\'change\').parent().show(); }
              if (this.value == \'external\') { alert(\'ATTENTION! Before using this option, Make sure that the ADMIN account that you setup matches at least one username on your external backend.  Otherwide you will lose Admin functionality.  If something messes up, edit config/config.php and change authType to either internal or both.\') }
              ',
              'options' => array(
                'Organizr' => 'internal',
            ...
            array(
              ...
              'name' => 'authBackend',
              'onchange' => '$(\'.be-auth\').each(function() { $(this).parent().hide(); }
            ); $(\'.be-auth-\'+this.value).each(function() { $(this).parent().show(); }
          );',
          'value' => AUTHBACKEND,
        ...
        array(
          ...
          'pattern' => '((?:[hH][tT][tT][pP]|[lL][dD][aA][pP]|[fF][tT][pP])[sS]?):\/\/([\w\.]{1,250}
        )(?::(\d{1,5}
      ))?((?:\/[^?.\s]+))?',
  ...
  'fields' => array(
    ...
    array(
      ...
      array(
        ...
        'icon' => 'gear',
        'onclick' => 'if ($(\'#git_branch_id[data-changed]\').length) { alert(\'Branch was altered, save settings first!\') }
        else { if (confirm(\''.translate('GIT_FORCE_CONFIRM').'\')) { performUpdate(); ajax_request(\'POST\', \'forceBranchInstall\'); }
        }
        ',
      ),
    ),
...
<div class="email-content info-box white-bg">
                        ...
                        <?php if (file_exists(FAIL_LOG)) {
                          ...
                          function getColor($colorTest)
                          {
                              if ($colorTest == "bad_auth") :
                                  ...
                                  $gotColorTest = "danger"; elseif ($colorTest == "good_auth") :
                          ...
                          foreach (array_reverse($gotFailLog["auth"]) as $key => $val) :
                                  ...
                                  if ($val["auth_type"] == "bad_auth") : $badLogin++; elseif ($val["auth_type"] == "good_auth") : $goodLogin++;
                          endif; ?>
  ...
  $(".generateEmails").click(function() {
    ...
    ajax_request('POST', 'get-emails', {type : backend}
  ).done(function(data){
    console.log('start');
