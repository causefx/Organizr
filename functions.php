...
function resolvePlexItem($server, $token, $item, $nowPlaying = false, $showNames = false, $role = false, $playlist = false) {
    ...
    // Assemble Item And Cache Into Array
    if($nowPlaying){
		$musicOverlay = ($widthOverride =- 56) ? '<img class="" style="width: 99%;display:block;position: absolute;top: 0;height: fit-content;overflow: hidden;filter:blur(3px);" src="'.$image_url.'">' : '';
