...
function resolvePlexItem($server, $token, $item, $nowPlaying = false, $showNames = false, $role = false, $playlist = false) {
    ...
    // Assemble Item And Cache Into Array
    if($nowPlaying){
		$musicOverlay = ($widthOverride =- 56) ? '<img class="" style="width: 98%;display:block;position: absolute;top: 1px;left:5px;height: fit-content;overflow: hidden;filter:blur(1px);" src="'.$image_url.'">' : '';
